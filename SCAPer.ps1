...
#Control if we want to check the STIGs on the machine, or find what Rule_ID's are mssing from our script.
$SCAP = $true
...
#if ($SCAP -eq $false) {$cklFile = Get-Item "C:\Users\1456084571E\Documents\PS Workshop\SCAP\2008MemberServer.ckl"} #For testing
$cklFileName = $cklFile.name.split(".")[0]
$cklFilePath = $cklFile.fullname
$checklist = Get-Content $cklFilePath
...
if ($SCAP -eq $true) {
    ...
    if ((Get-WmiObject -Class Win32_OperatingSystem).producttype -eq 2) {$DedicatedAD = $true}
        ...
        $title = "Dedicated to managing AD?"
        $message = "Is this system dedicated for managing Active Directory?"
        $yes = New-Object System.Management.Automation.Host.ChoiceDescription "&Yes","Yes"
        $no = New-Object System.Management.Automation.Host.ChoiceDescription "&No","No"
        $options = [System.Management.Automation.Host.ChoiceDescription[]]($No, $Yes)
        $result = $host.ui.PromptForChoice($title, $message, $options, 0)
        if ($result -eq 1) {$DedicatedAD = $true}
        else {$DedicatedAD = $false}
        }
    ...
    $DefaultDomainPolicy = (Get-GPO -Server $DC -DisplayName "Default Domain Policy" | Get-GPOReport -Server $DC -ReportType html).split("`n")
    #$Area52DefaultDomainPolicy = (Get-GPO -Server $DC -DisplayName "Area52 Default Domain Policy" | Get-GPOReport -Server $DC -ReportType html).split("`n")
...
:main for ($i = 0; $i -lt $checklist.count; $i++) {
    #If we found the beginning of a vuln listing, parse it
    if ($checklist[$i].trim() -eq "<VULN>") {
        #First, get all the info we need about the vuln
        $vulnInfo = New-Object PSObject
        :parse for (; $i -lt $checklist.count; $i++) {
            switch -Regex ($checklist[$i]) {
                "<VULN_ATTRIBUTE>" {
                    $AttributeName = $checklist[$i].Split(">")[1].Split("<")[0]
                    }
                "<ATTRIBUTE_DATA>" {
                    $AttributeData = $checklist[$i].Split(">")[1]
                    while ($AttributeData -notmatch "<") {
                        $i++
                        $AttributeData += "`n" + $checklist[$i]
                        }
                    $AttributeData = $AttributeData.Split("<")[0]
                    }
                "<STATUS>" {
                    $statusIndex = $i
                    $vulnInfo | Add-Member -MemberType NoteProperty -Name "Status" -Value ($checklist[$i].Split(">")[1].Split("<")[0])
                    }
                "</VULN>" {break parse}
                }
            #Check to see if we grabbed the Attribute names and info
            if ($AttributeName -ne $null -and $AttributeData -ne $null) {
                if ($ItemsToTrack -contains $AttributeName) {$vulnInfo | Add-Member -MemberType NoteProperty -Name $AttributeName -Value $AttributeData}
                Remove-Variable AttributeName,AttributeData
                }
            }
        #Now that we have all the info, we can actually check to see if its a finding
        <#
        Possible statuses:
        Open
        NotAFinding
        ...
        if ($vulnInfo.Status -ne "Not_Reviewed") {continue}
        ...
        <#
                ...
                "Rule_ID" { 
                    ...
                    #Comments descirbing the setting to configure and what to set it to
        ...
        #>
        Write-Host ("Checking Vuln " + $vulnInfo.Vuln_Num)
        if ($SCAP -eq $true) {
            #See which vuln we are checking, based on the Rule ID
            :rulecheck switch ($vulnInfo.Rule_ID) {
                #V-32025
                #Microsoft DotNet Framework 4.0 STIG :: Release: 6 Benchmark Date: 26 Oct 2018
                #Remoting Services TCP channels must utilize authentication and encryption.
                "SV-42341r1_rule" { ####UNTESTED####
                    #machine.config must exist, with typefilterlevel="Full" 
                    #if any [application].exe.config files exist, they must not have typefilterlevel=<anything but Full>
                    ...
                    #If there are none, this is a finding
                    if ($machineConfig.count -eq 0) {
                        $ActualStatus = "Open"
                        break rulecheck
                        }
                    #Check the machine.config files to ensure they have the required parameter
                    #I'm unsure as to how many occurances of typefilterlevel="Full" should exist.
                    foreach ($mchnCnfg in $machineConfig) {
                        #Import the config
                        $contents = Get-Content $mchnCnfg
                        #Get every line that sets typeFilterLevel
                        $lines = $contents -match "typeFilterLevel="
                        #If there are none, this is a finding
                        if (!$lines) {
                            $ActualStatus = "Open"
                            break rulecheck
                            }
                        #If typeFilterLevel is ever set to something other than Full, this is a finding
                        foreach ($line in $lines) {
                            if ($line -notlike "*typeFilterLevel=`"Full`"*") {
                                $ActualStatus = "Open"
                                break rulecheck
                                }
                            }
                        }
                    #Assuming we reach here, now we check the existing <application>.exe.config files to make sure they have the parameter too
                    #I'm not sure if it HAS to have the config as well (and if so, how many), or if it can not specify it and default from machine.config
                    #assuming the latter for now
                    $AppConfigs = Get-ChildItem "C:\" -Filter "*.exe.config" | select -ExpandProperty fullname
                    foreach ($config in $AppConfigs) {
                        #Import the config
                        $contents = Get-Content $config
                        #Get every line that sets typeFilterLevel
                        $lines = $contents -match "typeFilterLevel="
                        #If typeFilterLevel is ever set to something other than Full, this is a finding
                        foreach ($line in $lines) {
                    ...
                    #Since we've checked them all and haven't found a finding, then we're in the clear
                    if ($ActualStatus -ne "Open") {$ActualStatus = "NotAFinding"}
                    }
                #V-7055
                #Microsoft DotNet Framework 4.0 STIG :: Release: 5 Benchmark Date: 27 Jul 2018
                ...
                "SV-40966r1_rule" {
                    $ManualyVerify += $vulnInfo.Vuln_Num
                    }
                ...
                #V-46473
                #Microsoft Internet Explorer 11 Security Technical Implementation Guide :: Release: 16 Benchmark Date: 27 Jul 2018
                #Turn off Encryption Support must be enabled.
                "SV-59337r8_rule" {#here
                ...
                #V-1070
                #Windows Server 2012/2012 R2 Domain Controller Security Technical Implementation Guide :: Release: 13 Benchmark Date: 27 Jul 2018
                #Server systems must be located in a controlled access area, accessible only to authorized personnel.
                "SV-52838r1_rule" {
                    #You can't check this via the OS, this is a physical security item
                    $ManualyVerify += $vulnInfo.Vuln_Num
                    }
                ...
                "SV-53189r2_rule" {
                    #I've got no way to check what the latest SP it for the OS.  I suppose we could assume there wont be anymore and just check what we have
                    $ManualyVerify += $vulnInfo.Vuln_Num
                    }
                ...
                "SV-52841r2_rule" {
                    #I'm not sure, probably not a finding since we have RMAD
                    #verify
                    $ManualyVerify += $vulnInfo.Vuln_Num
                    }
                ...
                "SV-52843r3_rule" {
                    ...
                    #if (Get-Volume | Where {$_.DriveType -eq "Fixed" -and ($_.FileSystem -ne "NTFS" -and $_.FileSystem -ne "ReFS")}) {$ActualStatus = "Open"} #Get-Volume is PSv4+
...
"@
                ...
                "SV-52850r2_rule" {
                    ...
                    <#
                    $Value = Check-RegKeyValue "HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\RemoteAccess\Parameters\AccountLockout" "ResetTime (mins)"
                    if ($Value -ge "15") {$ActualStatus = "NotAFinding"}
                    else {$ActualStatus = "Open"}
                    #Verfiy, on member servers, this is for local accounts and not domain
                    #>
                    }
                ...
                "SV-52851r1_rule" {
                    ...
                    if ($Value -le 60 -and $Value -gt 0) {$ActualStatus = "NotAFinding"}
                    else {$ActualStatus = "Open"}
                    #>
                    }
                ...
                "SV-52855r1_rule" {
                    ...
                    #We're a DC, we don't have local accounts?
                    #This check is also the exact same for 2012 MS, so that must be why
                    <#
                    $Guest = @()
                    $Guest += Get-LocalGroupMember guests
                    if ($guest.count -eq 1) {
                        $localGuestName = $guest.name.Split("\")[1]
                        if ((Get-LocalUser $localGuestName).enabled -eq $false) {$ActualStatus = "NotAFinding"}
                        else {$ActualStatus = "Open"}
                        }
                    else {Write-Host "Verify V-1113 manually"}
                    #>
                    $ManualyVerify += $vulnInfo.Vuln_Num
                    }
                ...
                "SV-52856r1_rule" {
                    ...
                    #This check is also the exact same for 2012 MS, so that must be why
                    <#
                    $Guest = Get-LocalUser | Where {$_.name -eq "Guest"}
                    if ($Guest -eq $null) {$ActualStatus = "NotAFinding"}
                    else {$ActualStatus = "Open"}
                    #>
                    $ManualyVerify += $vulnInfo.Vuln_Num
                    }
                ...
                "SV-52857r1_rule" {
                    ...
                    #This check is also the exact same for 2012 MS, so that must be why
                    <#
                    $Admin = Get-LocalUser | Where {$_.name -eq "Administrator"}
                    if ($Admin -eq $null) {$ActualStatus = "NotAFinding"}
                    else {$ActualStatus = "Open"}
                    #>
                    $ManualyVerify += $vulnInfo.Vuln_Num
                    }
                ...
                "SV-52858r1_rule" {
                    ...
                    else {$ManualyVerify += $vulnInfo.Vuln_Num}
                    }
                ...
                "SV-52213r1_rule" {
                    try {
                        $printShares = get-printer * -full -EA Stop | Where {$_.shared -eq $true}
                        if ($printShares -eq $null) {$ActualStatus = "Not_Applicable"}
                        else {$ManualyVerify += $vulnInfo.Vuln_Num}
                        }
                    catch {$ManualyVerify += $vulnInfo.Vuln_Num}
                    }
                ...
                "SV-52863r2_rule" {
                    ...
                    #DCs don't have local accounts, its all in the domain (since the DCs host the domain)
                    #This is set in default domain policy, but the STIG says you have to check local settings.  Fuck.
                ...
                "SV-52864r3_rule" {
                    ...
                    $regpath = "registry::HKLM\System\CurrentControlSet\Control\Print\Providers\LanMan Print Services\Servers\"
                    $reg = Get-Item $regpath
                    if ($reg -eq $null) {$ActualStatus = "Open"}
                    else {
                        #How do i even check if its LESS restrictive?
                        $ManualyVerify += $vulnInfo.Vuln_Num
                        }
                    }
                ...
                "SV-52942r3_rule" {
                    ...
                    $DCbuiltinAdmin = Get-ADUser -SearchBase "CN=Users,DC=AREA52,DC=AFNOAPPS,DC=USAF,DC=MIL" -Filter * -Properties SID, PasswordLastSet | Where SID -Like "*-500" 
...
"@.Split("`n") | foreach {$_.trim()}
                ...
                "SV-51192r4_rule" {
                    #Lol yeah right.  This must be manually done, since I know we don't have everyone, and you cant script checking POAMs
                    $ManualyVerify += $vulnInfo.Vuln_Num
                    }
                ...
                "SV-53145r1_rule" {
                    ...
                    $Value = Check-RegKeyValue "HKLM\Software\Policies\Microsoft\Assistance\Client\1.0\" "NoExplicitFeedback" "SilentlyContinue"
                    if ($Value -eq "1") {$ActualStatus = "NotAFinding"}
                    else {$ActualStatus = "Open"}
                    }
                ...
                "SV-53179r2_rule" {
                    ...
                    #if (!($Value -band 1) -and !($value -band 2)) {$ActualStatus = "NotAFinding"} #2147483640 would be without RC4 #This check is stupid, since I would have to check EVERY bit for Future Encryption Types
                    if ($value -eq "2147483644") {$ActualStatus = "NotAFinding"} #2147483640 would be without RC4
                    else {$ActualStatus = "Open"}
                    }
                ...
                "SV-52880r1_rule" {
                    ...
                    <# Maybe?
                    $value = ($DefaultDomainPolicy -match "Store passwords using reversible encryption").replace("<td>",";").split(";")[2].split("<")[0]
                ...
                "SV-51160r2_rule" {
                    ...
                    $value = ($DefaultDomainPolicy -match "Enforce user logon restrictions").replace("<td>",";").split(";")[2].split("<")[0]
                    if ($Value -eq "Enabled") {$ActualStatus = "NotAFinding"}
                    else {$ActualStatus = "Open"}
                    }
                ...
                "SV-51162r2_rule" {
                    ...
                    #Can't be done via registry, need to pull GPO report and look in there.
                    #Verify if shows in RSOP when not showing in gpresult
                    $value = ($DefaultDomainPolicy -match "Maximum lifetime for service ticket").replace("<td>",";").split(";")[2].split("<")[0]
                    $num = [int]$value.split(" ")[0]
                ...
                "SV-51164r2_rule" {
                    ...
                    #Can't be done via registry, need to pull GPO report and look in there.
                    $value = ($DefaultDomainPolicy -match "Maximum lifetime for user ticket").replace("<td>",";").split(";")[2].split("<")[0]
                    $num = [int]$value.split(" ")[0]
                ...
                "SV-51166r2_rule" {
                    ...
                    $value = ($DefaultDomainPolicy -match "Maximum lifetime for user ticket renewal").replace("<td>",";").split(";")[2].split("<")[0]
                    $num = [int]$value.split(" ")[0]
                ...
                "SV-51168r3_rule" {
                    ...
                    $value = ($DefaultDomainPolicy -match "Maximum tolerance for computer clock synchronization").replace("<td>",";").split(";")[2].split("<")[0]
                    $num = [int]$value.split(" ")[0]
                ...
                "SV-53118r1_rule" {
                    ...
                    #Administrators 
                    #Local Service 
                    $value = ($GPResult -match "<tr><td>Change the system time").replace("<td>",";").split(";")[2].split("<")[0]
                    $ValArray = $value.Split(",") | foreach {$_.trim()}
                ...
                "SV-53039r3_rule" {
                    ...
                    $ValArray = $value.Split(",") | foreach {$_.trim()}
                    if ($valArray -contains "Administrators") {
                        if ($valArray.count -eq 1) {$ActualStatus = "NotAFinding"}
                        elseif ($valArray.count -eq 2 -and $valArray -contains "Auditors") {$ActualStatus = "NotAFinding"}
                        else {
                            $otherEntries = $valArray | Where {$_ -ne "Administrators" -and $_ -ne "Auditors"}

                            $title = "Documentation?"
                            $message = "Do we have documentation for the following entries to have `"Manage auditing and security log`" rights?`n" + ($otherEntries -join "`n")
                            $yes = New-Object System.Management.Automation.Host.ChoiceDescription "&Yes","Yes"
                            $no = New-Object System.Management.Automation.Host.ChoiceDescription "&No","No"
                            $options = [System.Management.Automation.Host.ChoiceDescription[]]($No, $Yes)
                            $result = $host.ui.PromptForChoice($title, $message, $options, 0)
                ...
                "SV-53033r1_rule" {
                    ...
                    $value = ($GPResult -match "<tr><td>Modify an object label").replace("<td>",";").split(";")[2].split("<")[0]
                    if ($Value -eq "") {$ActualStatus = "NotAFinding"}
                    else {$ActualStatus = "Open"}
                    }
                ...
                "SV-53019r1_rule" {
                    ...
                    $value = ($GPResult -match "<tr><td>Profile system performance").replace("<td>",";").split(";")[2].split("<")[0]
                    $ValArray = $value.Split(",") | foreach {$_.trim()}
                ...
                "SV-52121r2_rule" {
                    ...
                    #Local Service
                    #Network Service 
                    $value = ($GPResult -match "<tr><td>Replace a process level token").replace("<td>",";").split(";")[2].split("<")[0]
                    $ValArray = $value.Split(",") | foreach {$_.trim()}
                    if ($ValArray.Count -eq 2 -and $valArray -contains "Local Service" -and $valArray -contains "Network Service") {$ActualStatus = "NotAFinding"}
                    else {$ActualStatus = "Open"}
                    }
                ...
                "SV-52957r6_rule" {
                    if ($IsNIPR) {
                        Remove-Variable certs,HasDodRoot2,HasDodRoot3,HasDodRoot4 -EA SilentlyContinue
                        $certs = Get-ChildItem -Path Cert:Localmachine\disallowed | Where {$_.Issuer -Like "*DoD Interoperability*" -and $_.Subject -Like "*DoD*"}
                        $HasDodRoot2 = $certs | Where {$_.subject -eq "CN=DoD Root CA 2, OU=PKI, OU=DoD, O=U.S. Government, C=US" -and $_.Issuer -eq "CN=DoD Interoperability Root CA 1, OU=PKI, OU=DoD, O=U.S. Government, C=US" -and $_.Thumbprint -eq "22BBE981F0694D246CC1472ED2B021DC8540A22F" -and $_.NotAfter -gt $currDate}
                        $HasDodRoot3 = $certs | Where {$_.subject -eq "CN=DoD Root CA 3, OU=PKI, OU=DoD, O=U.S. Government, C=US" -and $_.Issuer -eq "CN=DoD Interoperability Root CA 2, OU=PKI, OU=DoD, O=U.S. Government, C=US" -and $_.Thumbprint -eq "FFAD03329B9E527A43EEC66A56F9CBB5393E6E13" -and $_.NotAfter -gt $currDate}
                        $HasDodRoot4 = $certs | Where {$_.subject -eq "CN=DoD Root CA 3, OU=PKI, OU=DoD, O=U.S. Government, C=US" -and $_.Issuer -eq "CN=DoD Interoperability Root CA 2, OU=PKI, OU=DoD, O=U.S. Government, C=US" -and $_.Thumbprint -eq "FCE1B1E25374DD94F5935BEB86CA643D8C8D1FF4" -and $_.NotAfter -gt $currDate}
                        if ($HasDodRoot2 -and $HasDodRoot3 -and $HasDodRoot4) {$ActualStatus = "NotAFinding"}
                        else {$ActualStatus = "Open"}
                        }
                    else {$ActualStatus = "Not_Applicable"}
                    }
                ...
                "SV-52956r3_rule" { 
                    ...
                    if ($regAcl.Access.count -eq 5 -and $adminDefault -eq $true -and $SYSTEMDefault -eq $true -and $CREATOROWNERDefault -eq $true -and $UsersDefault -eq $true -and $AllAppPkgsDefault -eq $true) {$32bitFine = $true}
                ...
                "SV-52881r3_rule" {
                    ...
                    #Judgment call
                    $NonDefaultShares = Get-SmbShare | where {$_.name -ne "ADMIN$" -and $_.name -ne "C$" -and $_.name -ne "IPC$"}
                ...
                "SV-51177r5_rule" {
                    #verify manually, idk how to check minimum permissions
                    $ManualyVerify += $vulnInfo.Vuln_Num
                    }
                ...
                "SV-52893r1_rule" {
                    ...
                    #This is set in the area52 default domain policy, but the STIG says to check locally. Fuck.
                ...
                "SV-51578r2_rule" {
                    #This should be covered by forcing everyone to use the proxy, and the proxy blocks internet traffic for admins.
                    #Leave it to be manually verified
                    $ManualyVerify += $vulnInfo.Vuln_Num
                    }
                ...
                "SV-51576r1_rule" {
                    #Documentation STIG.  Need separate 2875s, one for use and one for admin
                    $ManualyVerify += $vulnInfo.Vuln_Num
                    }
                ...
                "SV-51579r1_rule" {
                    #This is set by default domain policy, all account need min of 15 character passwords
                    $value = ($DefaultDomainPolicy -match "Minimum password length").replace("<td>",";").split(";")[2].split("<")[0].split(" ")[0]
                    if ($Value -ge "15") {
                        #Then we need documented policy
                    
                        $title = "Application Account password min characters?"
                        $message = "Do we have policy documented requiring Application Accounts to have passwords at least 15 characters long?"
                        $yes = New-Object System.Management.Automation.Host.ChoiceDescription "&Yes","Yes"
                        $no = New-Object System.Management.Automation.Host.ChoiceDescription "&No","No"
                        $options = [System.Management.Automation.Host.ChoiceDescription[]]($No, $Yes)
                        $result = $host.ui.PromptForChoice($title, $message, $options, 0)
                ...
                "SV-51580r3_rule" {
                    #HAHAHAHAHHAHAHAHHAHAHHHAHAHAHAHAHHA
                    $ManualyVerify += $vulnInfo.Vuln_Num
                    }
                ...
                "SV-51577r1_rule" {
                    #Policy STIG.  Cant be verified by script
                    $ManualyVerify += $vulnInfo.Vuln_Num
                    }
                ...
                "SV-51569r1_rule" {
                    #Idk how to check this with 100% certainty
                    $ManualyVerify += $vulnInfo.Vuln_Num
                    }
                ...
                "SV-52135r3_rule" {
                    #Idk how to check least restrictive
                    $ManualyVerify += $vulnInfo.Vuln_Num
                    }
                ...
                "SV-52196r5_rule" {
                    if ($IsNIPR) {
                        Remove-Variable certs,HasDodRoot2,HasDodRoot3 -EA SilentlyContinue
                        $certs = Get-ChildItem -Path Cert:Localmachine\disallowed | Where {$_.Issuer -Like "*CCEB Interoperability*"}
                        $HasDodRoot2 = $certs | Where {$_.subject -eq "CN=DoD Root CA 2, OU=PKI, OU=DoD, O=U.S. Government, C=US" -and $_.Issuer -eq "CN=US DoD CCEB Interoperability Root CA 1, OU=PKI, OU=DoD, O=U.S. Government, C=US" -and $_.Thumbprint -eq "DA36FAF56B2F6FBA1604F5BE46D864C9FA013BA3" -and $_.NotAfter -gt $currDate}
                        $HasDodRoot3 = $certs | Where {$_.subject -eq "CN=DoD Root CA 3, OU=PKI, OU=DoD, O=U.S. Government, C=US" -and $_.Issuer -eq "CN=US DoD CCEB Interoperability Root CA 2, OU=PKI, OU=DoD, O=U.S. Government, C=US" -and $_.Thumbprint -eq "929BF3196896994C0A201DF4A5B71F603FEFBF2E" -and $_.NotAfter -gt $currDate}
                        if ($HasDodRoot2 -and $HasDodRoot3) {$ActualStatus = "NotAFinding"}
                        else {$ActualStatus = "Open"}
                        }
                    else {$ActualStatus = "Not_Applicable"}
                    }
...
"@.split("`n") | Foreach {$_.trim()}
                ...
                "SV-72063r2_rule" {
                    ...
                    for ($i = 0; $i -lt $ous.count; $i++) {
                        $ou = $ous[$i]
                        Write-Progress -Activity ("Getting temp accounts (OU " + ($i + 1) + " out of " + $ous.count + ")" )  -Status "Checking $ou" -PercentComplete (($i+1)/$ous.count * 100)
                        while ($success -ne $true) {
                            try {
                                $temp = Search-ADAccount -SearchBase $ou -SearchScope OneLevel -AccountExpiring -TimeSpan 3:00:00:00 -EA stop
                                $success = $true
                                }
                            catch {$success = $false}
                            }
                    ...
                    if ($users.Count -eq 0) {$ActualStatus = "Not_Applicable"}
                    else {
                        foreach ($user in $users) {
                            $account = get-aduser $user -Properties AccountExpirationDate,whencreated
                            if ($account.whencreated.adddays(3) -lt $account.AccountExpirationDate) {$ActualStatus = "Open"}
                            }
                            if ($ActualStatus -eq $null) {$ActualStatus = "NotAFinding"}
                        }
                    }
                ...
                "SV-72065r3_rule" {
                    ...
                    if ($users.Count -eq 0) {$ActualStatus = "Not_Applicable"}
                        ...
                        $title = "Emergency Admins?"
                        $message = "Are any of the listed accounts Emergency Administrator Accounts?"
                        $yes = New-Object System.Management.Automation.Host.ChoiceDescription "&Yes","Yes"
                        $no = New-Object System.Management.Automation.Host.ChoiceDescription "&No","No"
                        $options = [System.Management.Automation.Host.ChoiceDescription[]]($No, $Yes)
                        $result = $host.ui.PromptForChoice($title, $message, $options, 0)
                ...
                "SV-72135r2_rule" {
                    #Personally I'm not confident in relying on default permissions to be the tell if its compliant
                    $ManualyVerify += $vulnInfo.Vuln_Num
                    }
                ...
                "SV-52938r2_rule" {
                    ...
                    #STIG says you have to check local settings.  FUCK
                ...
                "SV-52940r2_rule" {
                    ...
                    <#Run this to find which accounts are offending
                    $ous = Get-ADOrganizationalUnit -filter * | select -ExpandProperty distinguishedname
                    $users = @()
                    for ($i = 0; $i -lt $ous.count; $i++) {
                        $ou = $ous[$i]
                        ...
                        while ($success -ne $true) {
                            try {
                                #Live environment, OUs can be deleted/moved/renamed while we're running
                                if (Get-ADOrganizationalUnit $ou) {
                                    $temp = Get-ADUser -Filter {Enabled -eq $true -and PasswordNotRequired -eq $true} -SearchBase $ou -SearchScope OneLevel -properties samaccountname,canonicalname,enabled,PasswordNotRequired,displayname,name,o,extensionattribute13,userworkstations -EA Stop
                                    }
                                $success = $true
                                }
                            catch {$_;$success = $false}
                    ...
                    $users | sort canonicalname | select samaccountname,canonicalname,enabled,PasswordNotRequired,displayname,name,o,extensionattribute13,userworkstations
                    #>
                    }
                ...
                "SV-88471r2_rule" {
                    #if ((Get-WindowsOptionalFeature -Online | Where FeatureName -eq SMB1Protocol).state -ne "enabled") {$ActualStatus = "NotAFinding"} #Get-WindowsOptionalFeature is PSv4+
                    if ((Get-WmiObject Win32_OptionalFeature -Filter "name = 'SMB1Protocol'").InstallState -ne 1) {$ActualStatus = "Not_Applicable"}
                    else {$ActualStatus = "Open"}
                    }
                ...
                "SV-90603r1_rule" {
                    ...
                    for ($i = $StartIndex; $i -lt $EndIndex; $i++) {
                        ...
                        if ($line -match "S-\d-\d+-(\d+-){1,14}\d+") {
                            if ($found -eq $false) {Write-Host "The following User Rights Assignments have Orphaned SIDs defined in them:"}
                            $line.split(">")[2].split("<")[0] + " (" + $line.split(">")[6].split("<")[0] + ")"
                            $ActualStatus = "Open"
                            $found = $true
                            }
                        }
                ...
                "SV-95185r1_rule" {
                    if ((Get-WindowsFeature -Name PowerShell-v2).Installed -ne $true) {$ActualStatus = "NotAFinding"}
                    else {$ActualStatus = "Open"}
                    }
                ...
                "SV-51175r3_rule" {
                    #Idk how to check for least restrictive
                    $ManualyVerify += $vulnInfo.Vuln_Num
                    }
                ...
                "SV-51180r2_rule" {
                    ...
                    #Idk if base share folders count, but i think they do
                    $ManualyVerify += $vulnInfo.Vuln_Num
                    }
...
"@.split("`n") | foreach {$_.trim()}
                    ...
                    if ($ActualStatus -ne "Open" -and (Get-Service -DisplayName "Windows Time").StartType -ne "Automatic") {
                    
                        $title = "Alt timesource?"
                        $message = "Is the alternate time sync source configured to start automatically?"
                        $yes = New-Object System.Management.Automation.Host.ChoiceDescription "&Yes","Yes"
                        $no = New-Object System.Management.Automation.Host.ChoiceDescription "&No","No"
                        $options = [System.Management.Automation.Host.ChoiceDescription[]]($No, $Yes)
                        $result = $host.ui.PromptForChoice($title, $message, $options, 0)
                        if ($Value -eq "0") {$ActualStatus = "Open"}
                        }
                    if ($ActualStatus -ne "Open") {$ActualStatus = "NotAFinding"}
                    }
                
                #V-1155
                #Windows Server 2012/2012 R2 Member Server Security Technical Implementation Guide :: Release: 2.13 Benchmark Date: 27 Jul 2018
                ...
                "SV-51499r3_rule" { 
                    ...
                    $value = ($GPResult -match "<tr><td>Access this computer from the network").replace("<td>",";").split(";")[2].split("<")[0]
                    $valArray = $value.split(",") | foreach {$_.trim()}
                    if ($valArray -contains "Administrators" -and (($valArray -contains "Authenticated Users" -and $DedicatedAD -eq $false) -or ($DedicatedAD -eq $true -and $valArray -notcontains "Authenticated Users"))) {$ActualStatus = "NotAFinding"}
                    else {$ActualStatus = "Open"}
                    }
                ...
                "SV-83319r1_rule" { 
                    ...
                    $NeedISSO = @()
                    if ($valArray -contains "Administrators") {
                        if ($valArray -contains "Remote Desktop Users") {
                            if ((Get-WindowsFeature -Name Remote-Desktop-Services).InstallDate -ne "Installed") {$ActualStatus = "Open"}
                            else {
                                if ($valArray.Count -eq 2) {$ActualStatus = "NotAFinding"}
                                else {$NeedISSO += $valArray | Where {$_ -ne "Administrators" -and $_ -ne "Remote Desktop Users"}}
                                }
                            }
                        elseif ($valArray.count -eq 1) {$ActualStatus = "NotAFinding"}
                        else {$NeedISSO += $valArray | Where {$_ -ne "Administrators"} }
                        if ($NeedISSO.count -gt 0) {
                            
                            $title = "ISSO documented?"
                            $message = "Are the following groups documented with the ISSO to have the `"Allow log on through Remote Desktop Services`" user rights assignment (per V-26473)?`n" + ($NeedISSO -join "`n")
                            $yes = New-Object System.Management.Automation.Host.ChoiceDescription "&Yes","Yes"
                            $no = New-Object System.Management.Automation.Host.ChoiceDescription "&No","No"
                            $options = [System.Management.Automation.Host.ChoiceDescription[]]($No, $Yes)
                            $result = $host.ui.PromptForChoice($title, $message, $options, 0)
                ...
                #V-1073
                #Windows Server 2008 R2 Member Server Security Technical Implementation Guide :: Release: 1.30 Benchmark Date: 27 Jul 2018
                ...
                "SV-32248r1_rule" { 
                    #if (Get-Volume | Where {$_.DriveType -eq "Fixed" -and $_.FileSystem -ne "NTFS"}) {$ActualStatus = "Open"} #Get-volume is PSv4+
                    if (Get-WMIObject -Class Win32_Volume | Where {$_.DriveType -eq 3 -and $_.FileSystem -ne "NTFS"}) {$ActualStatus = "Open"}
                    else {$ActualStatus = "NotAFinding"}
                    }
...
"@
                ...
                "SV-32282r3_rule" { 
                    #Computer Configuration >> Windows Settings >> Security Settings >> Local Policies >> Security Options
                    #"Interactive Logon: Number of previous logons to cache (in case Domain Controller is not available)" to "2" logons or less. 
                    $Value = Check-RegKeyValue "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon\" "CachedLogonsCount"
                    if ($Value -le 2) {$ActualStatus = "NotAFinding"}
                    else {$ActualStatus = "Open"}
                    }
                ...
                "SV-32284r2_rule" { 
                    ...
                    #"Account lockout threshold" to "3" or less invalid logon attempts (excluding "0" which is unacceptable). 
                    #This is set in the default domain policy, but the STIG says to check locally. Fuck.
                    $value = ($GPResult -match "<tr><td>Account lockout threshold").replace("<td>",";").split(";")[2].split("<")[0]
                    $num = [int]$value.Split(" ")[0]
                    if ($num -le "3" -and $num -gt "0") {$ActualStatus = "NotAFinding"}
                    else {$ActualStatus = "Open"}
                    }
                ...
                "SV-32285r3_rule" { 
                    ...
                    #"Reset account lockout counter after" to at least "60" minutes. 
                    #This is set in the default domain policy, but the STIG says to check locally. Fuck.
                    $value = ($GPResult -match "<tr><td>Reset account lockout counter after").replace("<td>",";").split(";")[2].split("<")[0]
                    $num = [int]$value.Split(" ")[0]
                    if ($num -ge "15" -and $value -match "minutes") {$ActualStatus = "NotAFinding"}
                    else {$ActualStatus = "Open"}
                    }
                ...
                "SV-32286r3_rule" { 
                    ...
                    $value = ($GPResult -match "<tr><td>Account lockout duration").replace("<td>",";").split(";")[2].split("<")[0]
                    $num = [int]$value.Split(" ")[0]
                    if ($num -ge "15" -and $value -match "minutes") {$ActualStatus = "NotAFinding"}
                    else {$ActualStatus = "Open"}
                    <#I don't trust this
                    ...
                    #>
                    #Verfiy, might be for local account and not domain
                    }
                ...
                "SV-32289r2_rule" { 
                    ...
                    #"Minimum password age" to at least "1" day. 
                    #This is set in the default domain policy, but the STIG says to check locally. Fuck.
                    $value = ($GPResult -match "<tr><td>Minimum password age").replace("<td>",";").split(";")[2].split("<")[0]
                    $num = [int]$value.split(" ")[0]
                    if ($num -ge "1" -and $value -match "day") {$ActualStatus = "NotAFinding"}
                    else {$ActualStatus = "Open"}
                    }
                ...
                "SV-32290r3_rule" { 
                    #Computer Configuration >> Windows Settings >> Security Settings >> Account Policies >> Password Policy
                    #"Enforce password history" to "24" passwords remembered. 
                    $value = ($GPResult -match "<tr><td>Enforce password history").replace("<td>",";").split(";")[2].split("<")[0]
                    $num = [int]$value.split(" ")[0]
                    if ($num -ge "24") {$ActualStatus = "NotAFinding"}
                    else {$ActualStatus = "Open"}
                    }
                ...
                "SV-32298r2_rule" { 
                    #Computer Configuration >> Windows Settings -> Security Settings >> Account Policies >> Password Policy 
                    #"Password must meet complexity requirements" to "Enabled". 
                    $value = ($GPResult -match "<tr><td>Password must meet complexity requirements").replace("<td>",";").split(";")[2].split("<")[0]
                    if ($Value -eq "Enabled") {$ActualStatus = "NotAFinding"}
                    else {$ActualStatus = "Open"}
                    }
                ...
                #V-1155
                #Windows Server 2008 R2 Member Server Security Technical Implementation Guide :: Release: 1.30 Benchmark Date: 27 Jul 2018
                #The Deny access to this computer from the network user right on member servers must be configured to prevent access from highly privileged domain accounts and local accounts on domain systems and unauthenticated access on all systems.
                "SV-32302r6_rule" { 
                    #Computer Configuration >> Windows Settings >> Security Settings >> Local Policies >> User Rights Assignment
                    #"Deny access to this computer from the network" to include the following:
                    #Domain Systems Only:
                    ...
                    if ($valArray -contains "Guests") {
                        if ($PartOfDomain) {
                            if (($valArray -contains "Local account" -or $valArray -contains "Local account and member of Administrators group") -and (($valArray -match "\\Enterprise Admins" -and $valArray -match "\\Domain Admins") -or $DedicatedAD -eq $true)) {$ActualStatus = "NotAFinding"}
                            else {$ActualStatus = "Open"}
                            }
                        else {$ActualStatus = "NotAFinding"}
                        }
                ...
                "SV-32481r2_rule" { 
                    #Computer Configuration -> Windows Settings -> Security Settings -> Local Policies -> Security Options
                    #"Network access: Named pipes that can be accessed anonymously" to include no entries (blank). 
                    ...
                    $Value = Check-RegKeyValue "HKLM\System\CurrentControlSet\Services\LanManServer\Parameters\" "NullSessionPipes" 
                    if ($value -eq "" -or $value -eq $null) {$ActualStatus = "NotAFinding"}
                    else {
                        $title = "Documentation?"
                        $message = "Do we have documentation for the following named pipes to be accessed anonymously (V-3338)?`n" + ($value -join "`n")
                        $yes = New-Object System.Management.Automation.Host.ChoiceDescription "&Yes","Yes"
                        $no = New-Object System.Management.Automation.Host.ChoiceDescription "&No","No"
                        $options = [System.Management.Automation.Host.ChoiceDescription[]]($No, $Yes)
                        $result = $host.ui.PromptForChoice($title, $message, $options, 0)
                ...
                "SV-32320r1_rule" { 
                    #Computer Configuration -> Windows Settings -> Security Settings -> Local Policies -> Security Options
                    #“Devices: Allow Undock Without Having to Log On” to “Disabled”. 
                    $Value = Check-RegKeyValue "HKLM\Software\Microsoft\Windows\CurrentVersion\Policies\System" "UndockWithoutLogon"
                    if ($Value -eq "0") {$ActualStatus = "NotAFinding"}
                    else {$ActualStatus = "Open"}
                    }
...
"@.split("`n") | Foreach {$_.trim()}
                    ...
                    $Value = Check-RegKeyValue "HKLM\System\CurrentControlSet\Control\SecurePipeServers\Winreg\AllowedPaths\" "Machine" | sort
                    foreach ($item in $paths) {
                        if ($value -notcontains $item) {$item;$ActualStatus = "Open"}
                        }
                    if ($ActualStatus -ne "Open" -and $Value.count -eq 11) {$ActualStatus = "NotAFinding"}
                    elseif ($ActualStatus -ne "Open" -and $Value.count -gt 11) {
                        $title = "ISSO documented?"
                ...
                "SV-32369r1_rule" { 
                    ...
                    #"Minimum password length" to "14" characters. 
                    #This is set in the default domain policy, but the STIG says to check locally. Fuck.
                    $value = ($GPResult -match "<tr><td>Minimum password length").replace("<td>",";").split(";")[2].split("<")[0].split(" ")[0]
                    if ($Value -ge "14") {$ActualStatus = "NotAFinding"}
                    else {$ActualStatus = "Open"}

                    <# I don't think this works
                    $Value = Check-RegKeyValue "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\Network" "MinPwdLen" "SilentlyContinue"
                    if ($Value -ge "14") {$ActualStatus = "NotAFinding"}
                    else {$ActualStatus = "Open"}
                    #>
                    }
                ...
                "SV-32398r1_rule" { 
                    ...
                    #Internet Communication setting ‘Turn off Internet download for Web publishing and online ordering wizards’ to “Enabled”. 
                    $Value = Check-RegKeyValue "HKLM\Software\Microsoft\Windows\CurrentVersion\Policies\Explorer" "NoWebServices"
                    if ($Value -eq "1") {$ActualStatus = "NotAFinding"}
                    else {$ActualStatus = "Open"}
                    }
                ...
                "SV-32414r1_rule" { 
                    #Computer Configuration -> Administrative Templates -> System -> Internet Communication Management
                    #Internet Communication settings “Turn off the “Order Prints” picture task” to “Enabled”. 
                    $Value = Check-RegKeyValue "HKLM\Software\Microsoft\Windows\CurrentVersion\Policies\Explorer\" "NoOnlinePrintsWizard"
                    if ($Value -eq "1") {$ActualStatus = "NotAFinding"}
                    else {$ActualStatus = "Open"}
                    }
                ...
                "SV-32475r1_rule" { 
                    #Computer Configuration -> Administrative Templates -> Windows Components
                    #Game Explorer “Turn off downloading of game information” to “Enabled”. 
                    $Value = Check-RegKeyValue "HKLM\Software\Policies\Microsoft\Windows\GameUX\" "DownloadGameInfo"
                    if ($Value -eq "0") {$ActualStatus = "NotAFinding"}
                    else {$ActualStatus = "Open"}
                    }
                ...
                "SV-32430r1_rule" { 
                    ...
                    #“Configure Microsoft Spynet Reporting” to “Disabled”. 
                    $Value = Check-RegKeyValue "HKLM\SOFTWARE\Policies\Microsoft\Windows Defender\SpyNet\" "SpyNetReporting" "SilentlyContinue"
                    if ($Value -eq "1" -or $Value -eq "2") {$ActualStatus = "Open"}
                    else {$ActualStatus = "NotAFinding"}
                    }
                ...
                "SV-32706r3_rule" { 
                    ...
                    #This value is stored in bits.  The 1 bit is DES_CBC_CRC, the 2 bit is DES_CBC_MD5, the 4 bit is RC4_HMAC_MD5, the 8 bit is AES128_HMAC_SHA1, the 16 bit is AES256_HMAC_SHA1, and for "Future encryption types" they just turn on every other bit.
                    $Value = Check-RegKeyValue "HKLM\Software\Microsoft\Windows\CurrentVersion\Policies\System\Kerberos\Parameters\" "SupportedEncryptionTypes"
                    #if (!($Value -band 1) -and !($value -band 2)) {$ActualStatus = "NotAFinding"} #2147483640 would be without RC4 #This bit by bit check is stupid, since I would have to check them ALL for Future Encryption Types
                    if ($value -eq "2147483644") {$ActualStatus = "NotAFinding"} #2147483640 would be without RC4
                    else {$ActualStatus = "Open"}
                    }
                ...
                #V-21960
                #Windows Server 2008 R2 Member Server Security Technical Implementation Guide :: Release: 1.30 Benchmark Date: 27 Jul 2018
                #Domain users will be required to elevate when setting a networkGÇÖs location.
                "SV-32451r1_rule" { 
                    #Computer Configuration -> Administrative Templates -> Network -> Network Connections
                    #"Require domain users to elevate when setting a network's location" to "Enabled". 
                    $Value = Check-RegKeyValue "HKLM\Software\Policies\Microsoft\Windows\Network Connections\" "NC_StdDomainUserSetLocation"
                ...
                "SV-32461r1_rule" { 
                    ...
                    #“Turn off game updates” to “Enabled”. 
                    $Value = Check-RegKeyValue "HKLM\Software\Policies\Microsoft\Windows\GameUX\" "GameUpdateOptions"
                    if ($Value -eq "0") {$ActualStatus = "NotAFinding"}
                    else {$ActualStatus = "Open"}
                    }
                ...
                "SV-32462r1_rule" { 
                    ...
                    #“Prevent the computer from joining a homegroup” to “Enabled”. 
                    $Value = Check-RegKeyValue "HKLM\Software\Policies\Microsoft\Windows\Homegroup\" "DisableHomeGroup"
                    if ($Value -eq "1") {$ActualStatus = "NotAFinding"}
                    else {$ActualStatus = "Open"}
                    }
                ...
                "SV-32707r2_rule" { 
                    ...
                    #“Prevent Windows Anytime Upgrade from running” to “Enabled”. 
                    $Value = Check-RegKeyValue "HKLM\Software\Microsoft\Windows\CurrentVersion\Policies\Explorer\WAU\" "Disabled"
                    if ($Value -eq "1") {$ActualStatus = "NotAFinding"}
                    else {$ActualStatus = "Open"}
                    }
                ...
                "SV-33310r3_rule" { 
                    ...
                    #Idk how to check As Restrictive
                    $ManualyVerify += $vulnInfo.Vuln_Num
                    }
                ...
                "SV-33376r3_rule" { 
                    ...
                    elseif ($valArray.count -eq 1 -and $valArray -contains "Administrators") {
                ...
                "SV-83317r1_rule" { 
                    ...
                    if ($valArray -contains "Administrators") {
                        ...
                        if ($NeedISSO.count -gt 0) {
                            ...
                            $title = "ISSO documented?"
                            $message = "Are the following entries documented with the ISSO to have the `"Allow log on through Remote Desktop Services`" user rights assignment (per V-26473)?`n" + ($NeedISSO -join "`n")
                            $yes = New-Object System.Management.Automation.Host.ChoiceDescription "&Yes","Yes"
                            $no = New-Object System.Management.Automation.Host.ChoiceDescription "&No","No"
                            $options = [System.Management.Automation.Host.ChoiceDescription[]]($No, $Yes)
                            $result = $host.ui.PromptForChoice($title, $message, $options, 0)
                ...
                "SV-33484r4_rule" { 
                    ...
                    if ($valArray -contains "Administrators") {
                        ...
                        if ($NeedISSO -ne $null -and $otherEntries -ne "") {
                            $title = "Documentation?"
                            $message = "Are the following entries documented with the ISSO to have the `"Manage auditing and security log`" user rights assignment (per V-26496)?`n" + ($NeedISSO -join "`n")
                            $yes = New-Object System.Management.Automation.Host.ChoiceDescription "&Yes","Yes"
                            $no = New-Object System.Management.Automation.Host.ChoiceDescription "&No","No"
                            $options = [System.Management.Automation.Host.ChoiceDescription[]]($No, $Yes)
                            $result = $host.ui.PromptForChoice($title, $message, $options, 0)
                ...
                "SV-33491r2_rule" { 
                    ...
                    $value = ($GPResult -match "<tr><td>Profile system performance").replace("<td>",";").split(";")[2].split("<")[0]
                    $ValArray = $value.Split(",") | foreach {$_.trim()}
                    $otherEntries = $valArray | Where {$_ -ne "Administrators" -and $_ -ne "NT Service\WdiServiceHost"}
                    if ($otherEntries -eq "" -or $otherEntries -eq $null) {$ActualStatus = "NotAFinding"}
                    else {$ActualStatus = "Open"}
                    }
                ...
                "SV-42619r2_rule" { 
                    ...
                    if ($regAcl.Access.count -eq 4 -and $adminDefault -eq $true -and $SYSTEMDefault -eq $true -and $CREATOROWNERDefault -eq $true -and $UsersDefault -eq $true) {$32bitFine = $true}
                    ...
                    if ($OSArch -eq 64) {
                        ...
                        if ($regAcl.Access.count -eq 4 -and $adminDefault -eq $true -and $SYSTEMDefault -eq $true -and $CREATOROWNERDefault -eq $true -and $UsersDefault -eq $true) {$64bitFine = $true}
                        }
                    #If its not 64 bit, then we dont need to bother with it
                    else {$64bitFine = $true}
                ...
                #V-1072
                #Windows Server 2008 R2 Member Server Security Technical Implementation Guide :: Version 1, Release: 28 Benchmark Date: 26 Oct 2018
                #Shared user accounts must not be permitted on the system.
                "SV-32241r2_rule" { 
                    #Not checkable via script
                    $ManualyVerify += $vulnInfo.Vuln_Num
                ...
                #V-1074
                #Windows Server 2008 R2 Member Server Security Technical Implementation Guide :: Version 1, Release: 28 Benchmark Date: 26 Oct 2018
                #The Windows 2008 R2 system must use an anti-virus program.
                "SV-32244r4_rule" { 
                    if (Get-WmiObject -class Win32_Product -Filter "Name = 'McAfee Agent'") {$ActualStatus = "NotAFinding"}
                    else {$ActualStatus = "Open"}
                    }
                ...
                "SV-32246r2_rule" { 
                    #fuck me, i don't know how to logic this check
                    $ManualyVerify += $vulnInfo.Vuln_Num
                    }
                ...
                "SV-32294r1_rule" { 
                    $ScreenSaveActive = Check-RegKeyValue "HKCU\Software\Policies\Microsoft\Windows\Control Panel\Desktop\" "ScreenSaveActive"
                    $ScreenSaverIsSecure = Check-RegKeyValue "HKCU\Software\Policies\Microsoft\Windows\Control Panel\Desktop\" "ScreenSaverIsSecure"
                    $ScreenSaveTimeOut = Check-RegKeyValue "HKCU\Software\Policies\Microsoft\Windows\Control Panel\Desktop\" "ScreenSaveTimeOut"
                    if ($ScreenSaveActive -eq 1 -and $ScreenSaverIsSecure -eq 1 -and $ScreenSaveTimeOut -le 900) {$ActualStatus = "NotAFinding"}
                    else {$ActualStatus = "Open"}
                    }
                ...
                "SV-32263r1_rule" { 
                    #It says interview IAO, so not us
                    $ManualyVerify += $vulnInfo.Vuln_Num
                    }
                ...
                "SV-32268r1_rule" { 
                    #I have no idea how to query pending patches, especially considering it could be WSUS or SCCM
                    $ManualyVerify += $vulnInfo.Vuln_Num
                    }
                ...
                "SV-32269r2_rule" { 
                    ...
                    for ($i = 0; $i -lt $ous.count; $i++) {
                        ...
                        while ($success -ne $true) {
                            try {
                                $temp = Get-ADUser -Filter {Enabled -eq $true -and PasswordNeverExpires -eq $true -and smartcardlogonrequired -eq $false -and SID -notlike "*-500"} -SearchBase $ou -SearchScope OneLevel -EA Stop
                                $success = $true
                                }
                            catch {$success = $false}
                            }
                ...
                #V-7002
                #Windows Server 2008 R2 Member Server Security Technical Implementation Guide :: Version 1, Release: 28 Benchmark Date: 26 Oct 2018
                #Windows 2008 R2 accounts must be configured to require passwords.
                "SV-32270r2_rule" { 
                    $ous = Get-ADOrganizationalUnit -filter * | select -ExpandProperty DistinguishedName #| select ObjectGUID,DistinguishedName,name
                    #$users = @()
                    for ($i = 0; $i -lt $ous.count; $i++) {
                ...
                #V-14225
                #Windows Server 2008 R2 Member Server Security Technical Implementation Guide :: Version 1, Release: 28 Benchmark Date: 26 Oct 2018
                #The Windows 2008 R2 password for the built-in Administrator account must be changed at least annually or when a member of the administrative team leaves the organization.
                "SV-32274r2_rule" { 
                    #Get-ADUser -Filter * -Properties SID, PasswordLastSet | Where SID -Like "*-500" | FL Name, SID, PasswordLastSet
                    $DCbuiltinAdmin = Get-ADUser -SearchBase "CN=Users,DC=AREA52,DC=AFNOAPPS,DC=USAF,DC=MIL" -Filter * -Properties SID, PasswordLastSet | Where SID -Like "*-500" 
                    if ($DCbuiltinAdmin -eq $null) {
                ...
                "SV-32271r2_rule" { 
                    $AncientDate = (Get-Date).AddYears(-1)
                    [array]$AncientPasswords = Get-ADUser -SearchBase "OU=Service Accounts,OU=Administration,DC=AREA52,DC=AFNOAPPS,DC=USAF,DC=MIL" -Filter {enabled -eq $true -and PasswordLastSet -lt $AncientDate} -Properties PasswordLastSet | select name,passwordlastset
                    if ($AncientPasswords.count -eq 0) {$ActualStatus = "NotAFinding"}
                    else {$ActualStatus = "Open"}
                    }
                ...
                "SV-87389r1_rule" { 
                    #Computer Configuration >> Administrative Templates >> MS Security Guide
                    #"WDigest Authentication (disabling requires KB2871997)" to "Disabled".
                    $Value = Check-RegKeyValue "HKLM\SYSTEM\CurrentControlSet\Control\SecurityProviders\Wdigest\" "UseLogonCredential"
                    if ($Value -eq "0" -and (Get-HotFix -Id KB2871997 -EA SilentlyContinue)) {$ActualStatus = "NotAFinding"}
                    else {$ActualStatus = "Open"}
                    }

                #V-75915
                #Windows Server 2008 R2 Member Server Security Technical Implementation Guide :: Version 1, Release: 28 Benchmark Date: 26 Oct 2018
                #Orphaned security identifiers (SIDs) must be removed from user rights on Windows 2008 R2.
                "SV-90601r1_rule" { 
                    #Check gpresult for deleted accounts
                    #$GPResult -match "S(-([0-9]+?)+?)" | select -last 1
                    $StartIndex = $GPResult.indexof($GPResult -match "Local Policies/User Rights Assignment")
                ...
                #V-80473
                #Windows Server 2008 R2 Member Server Security Technical Implementation Guide :: Version 1, Release: 28 Benchmark Date: 26 Oct 2018
                #Windows PowerShell must be updated to a version that supports script block logging on Windows 2008 R2.
                "SV-95177r1_rule" { 
                    if ($PSVersionTable.PSVersion -like "4.0*" -or $PSVersionTable.PSVersion -match "5.*") {$ActualStatus = "NotAFinding"}
                    else {$ActualStatus = "Open"}
                    }
                ...
                #V-80475
                #Windows Server 2008 R2 Member Server Security Technical Implementation Guide :: Version 1, Release: 28 Benchmark Date: 26 Oct 2018
                #PowerShell script block logging must be enabled on Windows 2008 R2.
                "SV-95181r1_rule" { 
                    #Computer Configuration >> Administrative Templates >> Windows Components >> Windows PowerShell
                    #"Turn on PowerShell Script Block Logging" to "Enabled". 
                    #Install patch KB3109118 on Windows 2008 R2 on systems with PowerShell 4.0.
                    $Value = Check-RegKeyValue "HKLM\SOFTWARE\Policies\Microsoft\Windows\PowerShell\ScriptBlockLogging\" "EnableScriptBlockLogging" "SilentlyContinue"
                    if ($Value -eq "1" -and (Get-HotFix -Id KB3109118 -EA SilentlyContinue)) {$ActualStatus = "NotAFinding"}
                    else {$ActualStatus = "Open"}
                    }

                #V-1102
                #Windows Server 2012/2012 R2 Domain Controller Security Technical Implementation Guide :: Version 2, Release: 15 Benchmark Date: 08 Feb 2019
                ...
                "SV-52108r3_rule" { 
                    ...
                    $value = ($GPResult -match "<tr><td>Act as part of the operating system").replace("<td>",";").split(";")[2].split("<")[0]
                    $valArray = @()
                    $value.split(",") | Where {$_ -ne ""} | foreach {$valArray += $_.trim()}
                    if ($valArray.count -eq 0) {$ActualStatus = "NotAFinding"}
                    else {
                        $title = "ISSO documented?"
                        $message = "Are the following entries documented with the ISSO to have the `"Act as part of the operating system`" user rights assignment (per V-1102)?`n" + ($NeedISSO -join "`n")
                        $yes = New-Object System.Management.Automation.Host.ChoiceDescription "&Yes","Yes"
                        $no = New-Object System.Management.Automation.Host.ChoiceDescription "&No","No"
                        $options = [System.Management.Automation.Host.ChoiceDescription[]]($No, $Yes)
                        $result = $host.ui.PromptForChoice($title, $message, $options, 0)
                ...
                "SV-52115r3_rule" { 
                    ...
                    $NeedISSO = @()
                    if ($valArray -contains "Administrators") {
                        [Array]$NeedISSO = $valArray | Where {$_ -ne "Administrators"}
                        if ($NeedISSO.count -gt 0) {
                            $title = "ISSO documented?"
                            $message = "Are the following entries documented with the ISSO to have the `"Debug programs`" user rights assignment (per V-18010)?`n" + ($NeedISSO -join "`n")
                            $yes = New-Object System.Management.Automation.Host.ChoiceDescription "&Yes","Yes"
                            $no = New-Object System.Management.Automation.Host.ChoiceDescription "&No","No"
                            $options = [System.Management.Automation.Host.ChoiceDescription[]]($No, $Yes)
                            $result = $host.ui.PromptForChoice($title, $message, $options, 0)
                ...
                "SV-52111r3_rule" { 
                    ...
                    if ($valArray -contains "Administrators") {
                        ...
                        if ($NeedISSO.count -gt 0) {
                            $title = "ISSO documented?"
                            $message = "Are the following entries documented with the ISSO to have the `"Back up files and directories`" user rights assignment (per V-26474)?`n" + ($NeedISSO -join "`n")
                            $yes = New-Object System.Management.Automation.Host.ChoiceDescription "&Yes","Yes"
                            $no = New-Object System.Management.Automation.Host.ChoiceDescription "&No","No"
                            $options = [System.Management.Automation.Host.ChoiceDescription[]]($No, $Yes)
                            $result = $host.ui.PromptForChoice($title, $message, $options, 0)
                ...
                "SV-52113r3_rule" { 
                    ...
                    if ($valArray.count -eq 0) {$ActualStatus = "NotAFinding"}
                        ...
                        $title = "ISSO documented?"
                        $message = "Are the following entries documented with the ISSO to have the `"Create a token object`" user rights assignment (per V-26479)?`n" + ($NeedISSO -join "`n")
                        $yes = New-Object System.Management.Automation.Host.ChoiceDescription "&Yes","Yes"
                        $no = New-Object System.Management.Automation.Host.ChoiceDescription "&No","No"
                        $options = [System.Management.Automation.Host.ChoiceDescription[]]($No, $Yes)
                        $result = $host.ui.PromptForChoice($title, $message, $options, 0)
                ...
                "SV-52114r3_rule" { 
                    ...
                    $value = ($GPResult -match "<tr><td>Create global objects").replace("<td>",";").split(";")[2].split("<")[0]
                    $ValArray = $value.Split(",") | foreach {$_.trim()}
                    $NeedISSO = $valArray | Where {$_ -ne "Administrators" -and $_ -ne "Service" -and $_ -ne "Local Service" -and $_ -ne "Network Service"}
                    if ($NeedISSO.count -gt 0) {
                        $title = "ISSO documented?"
                        $message = "Are the following entries documented with the ISSO to have the `"Create global objects`" user rights assignment (per V-26480)?`n" + ($NeedISSO -join "`n")
                        $yes = New-Object System.Management.Automation.Host.ChoiceDescription "&Yes","Yes"
                        $no = New-Object System.Management.Automation.Host.ChoiceDescription "&No","No"
                        $options = [System.Management.Automation.Host.ChoiceDescription[]]($No, $Yes)
                        $result = $host.ui.PromptForChoice($title, $message, $options, 0)
                ...
                "SV-52116r3_rule" { 
                    ...
                    #Network Service 
                    #
                    #If an application requires this user right, this would not be a finding, but would need to be documented with the ISSO
                    $value = ($GPResult -match "<tr><td>Generate security audits").replace("<td>",";").split(";")[2].split("<")[0]
                    $ValArray = $value.Split(",") | foreach {$_.trim()}
                    $NeedISSO = $valArray | Where {$_ -ne "Local Service" -and $_ -ne "Network Service"}
                    if ($NeedISSO.count -gt 0) {
                        $title = "ISSO documented?"
                        $message = "Are the following entries documented with the ISSO to have the `"Generate security audits`" user rights assignment (per V-26489)?`n" + ($NeedISSO -join "`n")
                        $yes = New-Object System.Management.Automation.Host.ChoiceDescription "&Yes","Yes"
                        $no = New-Object System.Management.Automation.Host.ChoiceDescription "&No","No"
                        $options = [System.Management.Automation.Host.ChoiceDescription[]]($No, $Yes)
                        $result = $host.ui.PromptForChoice($title, $message, $options, 0)
                ...
                "SV-52117r3_rule" { 
                    ...
                    if ($NeedISSO.count -gt 0) {
                        $title = "ISSO documented?"
                        $message = "Are the following entries documented with the ISSO to have the `"Impersonate a client after authentication`" user rights assignment (per V-26490)?`n" + ($NeedISSO -join "`n")
                        $yes = New-Object System.Management.Automation.Host.ChoiceDescription "&Yes","Yes"
                        $no = New-Object System.Management.Automation.Host.ChoiceDescription "&No","No"
                        $options = [System.Management.Automation.Host.ChoiceDescription[]]($No, $Yes)
                        $result = $host.ui.PromptForChoice($title, $message, $options, 0)
                ...
                "SV-52118r3_rule" { 
                    ...
                    $value = ($GPResult -match "<tr><td>Increase scheduling priority").replace("<td>",";").split(";")[2].split("<")[0]
                    $ValArray = $value.Split(",") | foreach {$_.trim()}
                    $NeedISSO = $valArray | Where {$_ -ne "Administrators"}
                    if ($NeedISSO.count -gt 0) {
                        $title = "ISSO documented?"
                        $message = "Are the following entries documented with the ISSO to have the `"Increase scheduling priority`" user rights assignment (per V-26492)?`n" + ($NeedISSO -join "`n")
                        $yes = New-Object System.Management.Automation.Host.ChoiceDescription "&Yes","Yes"
                        $no = New-Object System.Management.Automation.Host.ChoiceDescription "&No","No"
                        $options = [System.Management.Automation.Host.ChoiceDescription[]]($No, $Yes)
                        $result = $host.ui.PromptForChoice($title, $message, $options, 0)
                ...
                "SV-52119r3_rule" { 
                    ...
                    #"Lock pages in memory" to be defined but containing no entries (blank). 
                    $value = ($GPResult -match "<tr><td>Lock pages in memory").replace("<td>",";").split(";")[2].split("<")[0]
                    $ValArray = $value.Split(",") | foreach {$_.trim()} | Where {$_ -ne ""}
                    $NeedISSO = $valArray | Where {$_ -ne "Administrators"}
                    if ($NeedISSO.count -gt 0) {
                        $title = "ISSO documented?"
                        $message = "Are the following entries documented with the ISSO to have the `"Lock pages in memory`" user rights assignment (per V-26494)?`n" + ($NeedISSO -join "`n")
                        $yes = New-Object System.Management.Automation.Host.ChoiceDescription "&Yes","Yes"
                        $no = New-Object System.Management.Automation.Host.ChoiceDescription "&No","No"
                        $options = [System.Management.Automation.Host.ChoiceDescription[]]($No, $Yes)
                        $result = $host.ui.PromptForChoice($title, $message, $options, 0)
                ...
                "SV-53039r4_rule" { 
                    ...
                    if ($valArray -contains "Administrators") {
                        ...
                        elseif ($valArray.count -eq 2 -and $valArray -contains "Auditors") {$ActualStatus = "NotAFinding"}
                            ...
                            $NeedISSO = $valArray | Where {$_ -ne "Administrators" -and $_ -ne "Auditors"}
                ...
                "SV-52122r3_rule" { 
                    ...
                    if ($NeedISSO.count -gt 0) {
                        $title = "ISSO documented?"
                        $message = "Are the following entries documented with the ISSO to have the `"Restore files and directories`" user rights assignment (per V-26504)?`n" + ($NeedISSO -join "`n")
                        $yes = New-Object System.Management.Automation.Host.ChoiceDescription "&Yes","Yes"
                        $no = New-Object System.Management.Automation.Host.ChoiceDescription "&No","No"
                        $options = [System.Management.Automation.Host.ChoiceDescription[]]($No, $Yes)
                        $result = $host.ui.PromptForChoice($title, $message, $options, 0)
                ...
                "SV-52123r3_rule" { 
                    ...
                    if ($NeedISSO.count -gt 0) {
                        $title = "ISSO documented?"
                        $message = "Are the following entries documented with the ISSO to have the `"Take ownership of files or other objects`" user rights assignment (per V-26506)?`n" + ($NeedISSO -join "`n")
                        $yes = New-Object System.Management.Automation.Host.ChoiceDescription "&Yes","Yes"
                        $no = New-Object System.Management.Automation.Host.ChoiceDescription "&No","No"
                        $options = [System.Management.Automation.Host.ChoiceDescription[]]($No, $Yes)
                        $result = $host.ui.PromptForChoice($title, $message, $options, 0)
                ...
                "SV-51596r2_rule" { 
                    ...
                    #"Interactive logon: Machine inactivity limit" to "900" seconds" or less. 
                    $Value = Check-RegKeyValue "HKLM\Software\Microsoft\Windows\CurrentVersion\Policies\System\" "InactivityTimeoutSecs"
                    if ($Value -le "900" -and $value -ne "0") {$ActualStatus = "NotAFinding"}
                    else {$ActualStatus = "Open"}
                    }
                ...
                "SV-101879r1_rule" { 
                    ...
                    $krbtgtSet = Get-ADUser krbtgt -Property PasswordLastSet | select -ExpandProperty PasswordLastSet #This check doesn't work, since this field isn't populated
                    if ($krbtgtSet -eq $null) {$ManualyVerify += $vulnInfo.Vuln_Num}
                    if ($krbtgtSet -gt $OldDate) {$ActualStatus = "NotAFinding"}
                    else {$ActualStatus = "Open"}
                    }

                #V-18395
                #Microsoft DotNet Framework 4.0 STIG :: Version 1, Release: 6 Benchmark Date: 26 Oct 2018
                #.Net Framework versions installed on the system must be supported.
                "SV-55642r1_rule" { 
                ...
                #V-1127
                #Windows Server 2012/2012 R2 Member Server Security Technical Implementation Guide :: Version 2, Release: 14 Benchmark Date: 26 Oct 2018
                ...
                "SV-51499r4_rule" { 
                    ...
                    $value = ($GPResult -match "<tr><td>Access this computer from the network").replace("<td>",";").split(";")[2].split("<")[0]
                    $valArray = $value.split(",") | foreach {$_.trim()}
                    $NeedISSO = $valArray | Where {$_ -ne "Administrators" -and $_ -ne "Authenticated Users"}
                    if ($NeedISSO.count -gt 0) {
                        $title = "ISSO documented?"
                        $message = "Are the following entries documented with the ISSO to have the `"Access this computer from the network`" user rights assignment (per V-26470)?`n" + ($NeedISSO -join "`n")
                        $yes = New-Object System.Management.Automation.Host.ChoiceDescription "&Yes","Yes"
                        $no = New-Object System.Management.Automation.Host.ChoiceDescription "&No","No"
                        $options = [System.Management.Automation.Host.ChoiceDescription[]]($No, $Yes)
                        $result = $host.ui.PromptForChoice($title, $message, $options, 0)
                ...
                #V-26485
                #Windows Server 2012/2012 R2 Member Server Security Technical Implementation Guide :: Version 2, Release: 14 Benchmark Date: 26 Oct 2018Windows Server 2012/2012 R2 Member Server Security Technical Implementation Guide :: Version 2, Release: 14 Benchmark Date: 26 Oct 2018
                #The Deny log on locally user right on member servers must be configured to prevent access from highly privileged domain accounts on domain systems, and from unauthenticated access on all systems.
                "SV-51508r3_rule" { 
                    #Computer Configuration -> Windows Settings -> Security Settings -> Local Policies -> User Rights Assignment
                    #"Deny log on locally" to include the following:
                ...
                "SV-32460r1_rule" { 
                    #Computer Configuration -> Administrative Templates -> Windows Components -> AutoPlay Policies 
                    #“Turn off Autoplay for non-volume devices” to “Enabled”.
                    $Value = Check-RegKeyValue "HKLM\Software\Policies\Microsoft\Windows\Explorer\" "NoAutoplayfornonVolume"
                    if ($Value -eq "1") {$ActualStatus = "NotAFinding"}
                    else {$ActualStatus = "Open"}
                    }
                ...
                default {
                    $NotChecked += $vulnInfo.Rule_ID
                    }
                }

            #And now that we've checked the vulnerability, update the checklist, IF we knew how to check it
            if ($ActualStatus -ne $null) {$checklist[$statusIndex] = $checklist[$statusIndex].Replace($vulnInfo.Status,$ActualStatus)}
            #reset our flag
            ...
            $Rule_ID = $vulnInfo.Rule_ID
            if (!($script -match $Rule_ID)) { #If we can find the Rule ID anywhere in the script, then we dont need to add it.
                $Vuln_ID = $vulnInfo.Vuln_Num
                $Rule_Title = $vulnInfo.Rule_Title
                $Benchmark = $vulnInfo.STIGRef
                $CopyToClip += @"
...
if ($SCAP -eq $true) {Out-File -InputObject $checklist -Encoding default "$env:USERPROFILE\desktop\Checklist_$env:computername`_$timestamp.ckl"}
else {
